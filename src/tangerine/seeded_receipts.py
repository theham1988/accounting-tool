...
class OcrProvider(Protocol):
    ...
    provider is deferred). Slice 03 does not exercise this boundary against a
    real provider, but `UploadedReceiptSource` composes any `OcrProvider` so
    the full upload -> OCR -> ExtractedReceipt path is testable with a fake.
    """
...
class UploadedReceiptSource:
    """Receipt source that composes raw uploads with an OCR provider.

    This is the upload -> OCR half of the flow: each raw blob is passed through
    the `OcrProvider` to produce an `ExtractedReceipt`. Tests supply a fake
    provider (a function returning a canned `ExtractedReceipt`); a real source
    will compose a Google Drive client with a real OCR provider. Direct upload
    and Drive import share this composition — only the source of `bytes`
    differs.
    """

    def __init__(
        self, uploads: list[bytes], ocr: OcrProvider
    ) -> None:
        self._uploads = list(uploads)
        self._ocr = ocr

    def receipts(self) -> list[ExtractedReceipt]:
        return [self._ocr.extract(raw) for raw in self._uploads]
