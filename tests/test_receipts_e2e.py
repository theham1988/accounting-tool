...
from tangerine.approvals import DecisionError, PriceBook, apply_decision
from tangerine.receipts import check_receipt
from tangerine.seeded_receipts import SeededReceiptSource, UploadedReceiptSource
...
def test_correct_decision_overrides_extracted_fields_before_storing(
    ...
    # A correction lands as the CORRECTED state, distinct from a plain APPROVED.
    assert decision.final_state == ReceiptState.CORRECTED
...
def test_corrected_purchase_reconciles_lines_plus_vat_to_total(
    day: date, beer_supplier: Supplier, chang_sku: Sku
) -> None:
    """A correction that changes line totals must not produce an internally
    inconsistent purchase.

    Bugbot finding: reusing the extracted total/vat after a correction that
    changed a unit price would store a Purchase whose lines + VAT no longer
    equal its total — breaking the same sum-check invariant the pipeline
    enforces everywhere else. The stored purchase must reconcile against the
    CORRECTED lines (total = sum(corrected lines) + vat).
    """
    skus = {chang_sku.sku_id: chang_sku}
    # Extracted: 1 unit at 40000 -> 40280 total. The OCR inflated the price.
    receipt = _receipt(
        beer_supplier.supplier_id,
        day,
        lines=(
    ...
    # Corrected: 1 unit at 4000. Total must recompute to 4000 + 280 = 4280.
    corrected = (
        ExtractedReceiptLine(
            description="Chang draught keg 30L",
            quantity=D("1"),
    ...
    decision = apply_decision(checked, _correct(corrected), PriceBook())

    assert decision.purchase is not None
    lines_total = sum(
        (pl.quantity * pl.unit_price for pl in decision.purchase.lines), D("0")
    )
    # The sum-check invariant holds against the corrected lines.
    assert lines_total + decision.purchase.vat == decision.purchase.total
    assert decision.purchase.total == D("4280")
    assert decision.purchase.vat == D("280")
...
def test_auto_rejected_receipt_cannot_be_approved(
    ...
    with pytest.raises(DecisionError):
        apply_decision(checked, _approve(), PriceBook())
...
# --- 7. true end-to-end: upload -> OCR -> check -> approve -> stored purchase


def test_full_flow_through_uploaded_source_with_fake_ocr(
    day: date, beer_supplier: Supplier, chang_sku: Sku
) -> None:
    """Worked example tracing the whole pipeline in one test.

    A raw upload (bytes) passes through a fake OCR provider into the checker,
    the receipt is queued, a partner approves it, and a Purchase is stored
    with last_known_price updated. This is the slice's namesake acceptance
    criterion: "feeds a synthetic receipt, asserts flag/reject/approve
    outcomes and stored purchase data".
    """
    extracted = _receipt(
        beer_supplier.supplier_id,
        day,
    ...
    # Fake OCR: returns the canned extraction regardless of input bytes.
    class _FakeOcr:
        def extract(self, raw: bytes) -> ExtractedReceipt:
            assert raw == b"<fake receipt bytes>"
            return extracted

    source = UploadedReceiptSource(
        uploads=[b"<fake receipt bytes>"], ocr=_FakeOcr()
    )
    skus = {chang_sku.sku_id: chang_sku}
    book = PriceBook()

    # source -> check (the only receipt the source yields).
    [one_extracted] = source.receipts()
    checked = check_receipt(one_extracted, skus=skus, reference_prices={})
    assert checked.state == ReceiptState.QUEUED

    # check -> approve -> stored purchase + price update.
    result = apply_decision(checked, _approve(), book)

    assert result.final_state == ReceiptState.APPROVED
